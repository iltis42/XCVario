	...
	if( pullMsg( s1_rx_q, s1) ){
		...
		if( blue_enable.get() == WL_BLUETOOTH )
			if( forwardMsg( s1, bt_tx_q ))
				ESP_LOGI(FNAME,"ttyS1 RX bytes %d forward to bt_tx_q", s1.length() );
		if( serial1_rxloop.get() )  // only 0=DISABLE | 1=ENABLE
			if( forwardMsg( s1, s1_tx_q ))
				ESP_LOGI(FNAME,"ttyS1 RX bytes %d looped to s1_tx_q", s1.length() );
		if( serial2_tx.get() & RT_S1 )
			if( forwardMsg( s1, s2_tx_q ))
				ESP_LOGI(FNAME,"ttyS1 RX bytes %d looped to s2_tx_q", s1.length() );
	}
}
	...
	if( pullMsg( s2_rx_q, s2) ){
		...
		if( blue_enable.get() == WL_WLAN )
			if( forwardMsg( s2, wl_aux_tx_q ))
				ESP_LOGI(FNAME,"ttyS2 RX bytes %d forward to wl_aux_tx_q port 8882", s2.length() );
		if( blue_enable.get() == WL_BLUETOOTH )
			if( forwardMsg( s2, bt_tx_q ))
				ESP_LOGI(FNAME,"ttyS2 RX bytes %d forward to bt_tx_q", s2.length() );
		if( serial2_tx.get() & RT_S1 )
			if( forwardMsg( s2, s1_tx_q ))
				ESP_LOGI(FNAME,"ttyS2 RX bytes %d looped to s1_tx_q", s2.length() );
	}
}
	...
	if( pullMsg( bt_rx_q, bt ) ){
		if( serial1_tx.get() & RT_WIRELESS )  // Serial data TX from bluetooth enabled ?
			if( forwardMsg( bt, s1_tx_q ) )
				ESP_LOGI(FNAME,"Send to  device, BT received %d bytes", bt.length() );
