    ...
    while (1)
    {
        ...
        if (gflags.inSetup != true)
        {
			...
			if( !(gflags.stall_warning_active || gflags.gear_warning_active || gflags.flarmWarning || gflags.gLoadDisplay || gflags.horizon )  ) {
				...
				Display->drawDisplay( Units::AirspeedRounded(airspeed), te_vario.get(), aTE, polar_sink, altitude.get(), t, batteryVoltage, s2f_delta, as2f, average_climb.get(), S2FSWITCH->getCruiseState(), gflags.standard_setting, flap_pos.get() );
			}
