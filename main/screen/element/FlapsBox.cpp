...
void FlapsBox::drawLabels(float lwk_speed, float uwk_speed)
{
    ...
    // alternatively, but not correctly working, because of a eglib bug with clipping and text output
    // int wk = (int)std::roundf(_flaps_position);
...
void FlapsBox::draw(float ias)
{
    ...
    if ( FLAP->haveSensor() ) {
        _last_flap_idx = flap_tmp; // keep in sync with actual position, option to not play any sound
    }
